...
def test_sharpa_domain_randomization_doc_matches_owner_config():
    ...
    owner_cfg = OmegaConf.load(
        root / "conf" / "ppo" / "task" / "sharpa" / "sharpa_inhand" / "mujoco.yaml"
    )
...
def test_check_training_entrypoint_semantics_accepts_current_patterns():
    ...
    doc_path = root / "README.md"
    content = """
uv run scripts/train_rsl_rl.py task=go1/go1_joystick_flat/mujoco algo.load_run=2026-01-01
...
```bash
uv run scripts/train_rsl_rl.py missing_key=true task=go1/go1_joystick_flat/mujoco
```
"""
...
def test_collect_doc_errors_scans_issue_templates_for_script_paths(tmp_path):
    ...
    issue_template.parent.mkdir(parents=True)
    issue_template.write_text(
        "placeholder: |\n  uv run scripts/missing_entrypoint.py task=go1/go1_joystick_flat/mujoco\n",
        encoding="utf-8",
    )
...
def test_collect_doc_errors_flags_unclosed_markdown_fence(tmp_path):
    ...
    readme.write_text(
        "```bash\nuv run scripts/train_rsl_rl.py task=go1/go1_joystick_flat/mujoco\n", encoding="utf-8"
    )
...
def test_collect_doc_errors_flags_python_script_invocation(tmp_path):
    readme = tmp_path / "README.md"
    readme.write_text(
        "```bash\nuv run python scripts/train_rsl_rl.py task=go1/go1_joystick_flat/mujoco\n```\n",
        encoding="utf-8",
    )

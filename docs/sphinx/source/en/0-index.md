...
| Go2 arm manip-loco | PPO, HIM-PPO | Committed MuJoCo owner YAMLs are present under `conf/ppo/task/go2/go2_arm_manip_loco/` and `conf/ppo_him/task/go2_arm_manip_loco/`. |
| Go2W joystick | PPO (torch, MLX configured) | PPO owner YAMLs exist for MuJoCo and Motrix flat/rough variants under `conf/ppo/task/go2w/go2w_joystick_*`. |
